...
# Font download URL
font_url='https://raw.githubusercontent.com/TecharyAdam/TecharyMacOSScripts/refs/heads/main/Fonts/DMSans/DMSansItalic.ttf'

# Check if the font file already exists in /Library/Fonts
if [ -f "/Library/Fonts/DMSansItalic.ttf" ]; then
  echo "Font already exists in /Library/Fonts. No need to download."
else
  # Download the font to a temporary location
  tmp_file="/tmp/DMSansItalic.TTF"
  ...
  if [ $? -eq 0 ]; then
    ...
    sudo chown root:wheel "/Library/Fonts/DMSansItalic.TTF"
